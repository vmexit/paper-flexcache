...
Adaptive algorithms failed to cover the full spectrum of access patterns because they rely on linear interpolation of patterns.
%
...
%
They alternate between two basic policies, and, what is more, if a basic policy targets more than one primitive access pattern, the two policies will interfere with each other.
%
...
%
Temporal adaptive algorithms invoke the LRU-Scan-Resistant and LFU-Churn-Resistant policies with probabilities, which are two linear interpolations implemented at different times.
%
The phase- and frequency-based classification can cover all four typical primitive access patterns and capture the mixed parts to fill the contiguous space.  

\TODO{figure for insight\label{fig:insight}}

This paper proposes a hotness- and phase-based type classification that covers all four typical primitive access patterns. 
%
...
\squishlist
\item{{\textbf{hotness and phase-based type classification}, which comprehensively characterizes primitive access patterns and reveals the relationship between them.}}
