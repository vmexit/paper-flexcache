...
\label{s:intro}
access frequency distribution for cache size aware policy.
...
%
\DZ{This is the background of the paper. I don't think they belong to intro.}
\XXX{In production environments, software cache systems have a fixed memory space and a specific cache replacement policy for all data sets.
%
...
Cache replacement policies are designed to \XXX{handle different access patterns adaptively} \DZ{I think this wording is imprecise. Cache replacement policies normally do not adapt to access patterns}.
%
...
%
As shown in figure~\ref{fig:cloudphysics}(a),  as the cache size increases, the \XXX{performance} \DZ{Vague, be more specific on what do you mean by performance} of S3FIFO gradually decreases from 95.7\% to 89.6\%, while the performance of LIRS steadily increases from 89.6\% to 96\%.
%
