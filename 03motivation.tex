...
To design a cache size-aware replacement policy, we need to condiser the cache size when we analyze the access pattern.
%
...
%
For the same workload slice, there are 12 pahses with 10\% WSS and 8 phases with 15\% WSS.
%
In figure~\ref{fig:wss}(a), half of objects accessed in the phase 0 are active in the rest phase, and the other half are inactive.
%
Frequently accessed objects in the phase 3 are active in phases 7 and 11.
%
When the cache size is larger, it contains more objects, and frequently accessed objects are concentrated.
%
In figure~\ref{fig:wss}(b), frequently accessed objects in the phase 0 are active in the rest phase with more accesses(darker color), and the objects in the phase 3 are active in the phase 5 and 7, which helps to discover the access pattern with cache size.
...
%
To optimize the basic version, we use a sliding window and an appropriate aging function to dynamically discover what objects are in the same phase.
%
...
\begin{figure}[t]
    ...
    \caption{Phase for cache size-aware.}
    \label{fig:exapmle}
\end{figure}
...
%
The more hits within a phase, the more suitable the access pattern is for caching, and it also indicates that the cache is relatively large with a higher hit ratio.
%
When we analyze the whole trace, it means the cache size is equal to the working set size with only a phase.
%
When we reduce the cache size, the number of phases in which frequently accessed object(\cc{FA-MP}) appears increases, and the access become either more concentrated(\cc{FA-FP}) or more evenly distributed across different phases(\cc{IF-MP}).
%
...
%
LeCaR adjust the policy based on the access in the ghost cache, and their miss ratio waves considerably when the cache size changes .
%
...
%
If the cache size is small, the phase 0 is important patterns, and the phase 4 is interference patterns.
%
As we mentioned in~(\autoref{ss:adaption-criteria}), policies store the filtered objects in the ghost cache, so when the cache size is close to 1MB, they can record the objects in pahse 4 to the ghost cache.
%
...
%
\sys integrates the phase information and the access frequency distribution to dynamically adjust the criteria~(\S\ref{ss:integrate}), which filters the objects in the phase 4 and keeps the objects in the phase 0.
%
With prefetching, \sys outperforms the other policies about 20\%.
...
%
We count the frequency at which an object is accessed and calculating the average resue time, discovering a linear-logarithmic relationship between them. 
%
...
%
In figure~\ref{fig:alibb}(a), we analyze the access and average reuse time distribution in Alibaba benchmark.
%
...
%
The distribution is the same as the one in figure~\ref{fig:alibb}(b), which is analyzed in Twitter benchmark.
...
%
Therefore, \sys speculatively retains these objects to get close to the optimal policy~(\autoref{ss:dueling}).
%
