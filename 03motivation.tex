\section{Identifying Workload Access Patterns}
\label{s:access-patterns}

This section presents how does \sys overcome the 

\subsection{Object-level Access Pattern Classification}

\PN{Prior attempts to classify access patterns.}
%
\DZ{Make important prior works, such as Cacheus and S3FIFO, as latex macro.}
\DZ{Explain what Cacheus does and why it fails}
\DZ{This may overlap with the content in \autoref{s:motivation}. Let's decide which place is the best to put the content upon writing.}

\PN{Hotness.}

\PN{Popularity.}

\DZ{Make "Hotness", "Hot", "Cold", "Popularity", "Popular", "Rare" as a macro, since we may change the term in the future.}

\subsection{Adjusting Thresholds Based on Cache Size}
...
%
To reduce the space overhead of recording all accesses in the sliding window, a FIFO queue with a hotness counter has a similar effect to the sliding window, which also hold the relative access sequence.
%
An object enters the head of FIFO queue when its phase begins.
%
Hotness analysis increases the hotness counter when hit occurs and ages the object at the tail of the queue, indicating the end of the phase.
%
As shown in figure~\ref{fig:Phase-based}(2), with footprint four, \cc{ABCDAB} fill the FIFO queue, and the aging function is halfing the hotness counter and evicting the object with a zero counter.
%
...
Data are dynamically classified into \textbf{frequent} and \textbf{infrequent} categories based on a hotness threshold determined within each phase.
%
To achieve accurate classification, each data item is assigned a hotness counter, and the overall distribution of these counters within the phase is collected as hotness information.
%
Without lose of generality, only a subset of data can reside in the cache, a target caching proportion is defined according to the system configuration.
%
All data items are then sorted in descending order of their hotness counters, and entries are progressively selected to fill the target cache portion.
%
The hotness counter corresponding to the boundary of this portion is designated as the hotness threshold, which serves to distinguish frequent data from infrequent data.
%
This threshold adapts across phases in response to variations in the target cache proportion, thereby maintaining classification consistency under different workload dynamics.

As shown in figure~\ref{fig:wss}, hotness changes with phase and phase size.
%
In the figure, a workload slice from Alibaba~\cite{} is split into phases with different phase sizes, and the hotness distribution is shown in heatmap.
%
\XXX{For visualization, we cap the maximum hotness counters at 4; values above 4 are still represented as 4, and we cut off some data accessed only once in this slice.}
%
...
%
The offline hotness analysis collects the hotness distribution and classifies data based on the threshold at the end of each phase.
%
In the online version, hotness distribution manages data in the FIFO queue for a phase.
%
When access or eviction occurs, it uses the new distribution to update the threshold dynamically.
%
With the new threshold, the online analysis classifies data into frequent and infrequent types.
%
It can also classify data when threshold changes due to access or eviction occurs to reduce the overhead.
...
%
This classification method covers recency property by hotness distribution in a phase and frequency property by the number of phases, and we will discuss spatial locality with prefetching in \TODO{ref}.
