...
%
To overcome this challenge, this paper proposes a new perspective to \textbf{adaptively} classify the workload into \textbf{cooperative} types based on \textit{phase and hotness} and converts it into an \textbf{online} version to guide the design of a reliable cache eviction algorithm in different cache sizes~(\TODO{cite design}).
...
%
More specifically, the number of accesses in a phase is changed adaptively, but the unique data in a phase is limited to a predefined footprint size.
%
...
\begin{figure}[t]
    ...
    With a large phase size as footprint six, \cc{A}, \cc{B}, and \cc{C} are persistent types for all phases, and others are ephemeral types.
    With a small phase size as footprint four, only \cc{A} appears in all phases as a persistent type, and \cc{B} and \cc{C} occur in some phases as persistent type compared to others as ephemeral types.
    }
    \label{fig:Phase-based}
\end{figure}

As shown in figure~\ref{fig:Phase-based}, we demonstrate how to split the trace into phases based on the footprint size and an online version with a sliding window for ~(\TODO{cite design}).
%
