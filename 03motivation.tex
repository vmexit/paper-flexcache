...
As shown in figure~\ref{fig:Phase-based}, we demonstrate how to split the trace into phases based on the footprint size and type the accesses dynamically in contiguous phase type space.
%
...
Splitting a workload into phases does not match the online algorithm's workflow, and we propose an optimized online version for \sys~(\TODO{cite}).
%

An optimized online version with a sliding window for ~(\TODO{cite design}).
