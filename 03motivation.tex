...
\subsection{The performance of the policy changes with cache size and access pattern}
...
As shown in the figure ~\autoref{fig:cloudphysics}, we evaluate the performance of the cache replacement policies XXX~\cite{} in Cloudphysics~\cite{} with the same relative cache size. (this is to state that prior work cannot do well in all access pattern in Cloudphysics)

As shown in the figure ~\autoref{fig:cache-size}, ARC~\cite{} and LIRS~\cite{} achieve different performance in different cache sizes in systor~\cite{}. (maybe this state average performance in different access patterns)
%
\begin{figure}[t]
    \centering
...
We use this cache size aware method to analyze the trace in Alibaba~\cite{} and find a different way to mark the object types. figure~\ref{fig:wss}
\begin{figure}[t]
    \centering
...
According to how many times the object accessed and how many phases it appears, we mark the object into four types dynamicly. 
...
\subsection{Dynamicly adjust the criteria based on the pahses information}
...
In the large cache size \dots

In the small cache size \dots
