...
One of the earliest adaptive algorithms is HAC~\cite{}\XXX{}, which dynamically adjusts its cache size between object-caching system and page-caching system.
%
Recent adaptive algorithms fall into two categories: 1) size-adaptive ones 
...
%
The representative examples are \arc~\cite{} and \car~\cite{}, both following 
the principle of dynamically balancing recency and frequency.
%
\arc divides the cache into two LRU lists: a recency list~(that tracks recently-accessed objects) and a frequency list~(that tracks 
frequently-accessed objects).
%
\DZ{Is this correct?}
\XXX{A newly accessed object is first placed in the recency list and enters the frequency list upon a second access.}
%
...
%
\DZ{Add one sentence here to explain what \car does.}
\car \XXX{}
...
A policy-adaptive algorithm adapts by switching between multiple base 
...
%
Both works involve using two base eviction algorithms and maintaining a weight for each algorithm. 
%
Upon a cache miss, they select one base algorithm to perform the eviction 
with probabilities proportional to the weights.
%
Next, they check if a wrongful eviction has occurred by finding if the wanted object was in the eviction history of either base algorithm.
%
If so, they penalize the corresponding algorithm by decreasing its weight. 
%
\XXX{Finally, both \cacheus and \lecar use machine learning to control the rate at which weights change.}
%
