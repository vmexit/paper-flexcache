...
\label{s:background}
This section presents a background on software cache systems~(\autoref{ss:background-software-cache}), and a phonomenon that most current replacement policies operate in a metastable state~(\autoref{ss:performance-policy-change}).
%
...
Software cache systems are widely implemented in various fields to improve service performance.
%
In storage systems, faster devices are typically more expensive and have smaller capacities. 
%
To reduce costs and minimize data access latency, we place a subset of the data on high-speed devices to form a cache system.
%
...
Generally, cache systems have three main research directions: \cc{access pattern analysis}, \cc{cache replacement policies}, and \cc{cache prefetching}. 
%
Access pattern analysis helps in designing replacement policies or adjusting parameters in real-time to adapt to different situations.
%
...
%
In production environments, the policy perform particularly poorly under certain characteristics, and increasing the cache size can sometimes lead to negative returns.
%
We evaluate \XXX{13} policies in \XXX{XX} traces with different cache sizes and calculate the relative hit ratio to the best policy in the policy set.
%
...
As shown in figure ~\autoref{fig:cloudphysics}, we evaluate the performance of \XXX{13} cache replacement policies~\cite{} in Cloudphysics~\cite{}. 
%
CloudPhysics provides \XXX{104} traces. 
%
We evaluate the hit ratio of replacement policies under a given cache size. 
%
...
%
While \sys is the best policy in all cache sizes, it is the \textbf{best} policy in \textbf{50\%} percent of traces, and achieves \textbf{95\%} performance in \textbf{95\%} of traces.
%


As shown in figure ~\autoref{fig:mrcdemo}, ARC~\cite{} and LIRS~\cite{} achieve the best performance in different cache sizes in systor~\cite{}. 
%
...
%
In figure ~\autoref{fig:mrcdemo}, when the cache size enlarges from 5GB to 6GB(\TODO{true cache size}), LIRS is the best policy, and others performance degrades about 5\%.
%
...
%
This classification method is very intuitive, and we determine the trace type only after the analysis (e.g., implementing the replacement policy).
