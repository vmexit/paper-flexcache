...
\DZ{Define these two types of algorithms.}

\PN{Static Eviction Algorithms.}
%
A static algorithm employs a fixed set of rules all the time to 
perform eviction.
%
A static algorithm has an insertion point when new data arrives,
an updating function when data is accessed, and a priority metric to decide
which data to evict when cache is full.
%
Some algorithms promote a lazy promotion~\cite{}, which delays the updating of data priority until eviction time.

\PN{Extending LRU lists.}
%
LRU is the most widely used static eviction algorithm, which inserts new data or updates accessed data to the head of list, and evicts data from the tail of list.
%
An extended LRU list proposes a new metric to measure data priority, modifying updating function.
%
\lhdc~\cite{} proposes a hit density metric and updates the priority of data when accessed.
%
Ideally, it will evict data with the lowest hit density.

\PN{Extending FIFO queues.}
FIFO-based queues leverage a lazy promotion, which holds a threshold to decide whether to promote data when eviction happens.
%
Clock~\cite{} uses a reference bit as the threshold, which promotes the tail data to the head of queue if the reference bit is set when eviction happens.
%
\sieve~\cite{} has a victim pointer scanning the queue from tail to head, evicting the first data whose access count is below a threshold.
%
\sthreefifo~\cite{} uses three FIFO queues 


%
%It has a fixed workflow whatever the access pattern is.
%
%As the basic LRU algorithm, it has a certain insertion point for new data, a updating function for accessed data, and an priority metric for eviction.
%
%S3-FIFO~\cite{}, LHD~\cite{}, and 2Q~\cite{} are typical static eviction algorithms.

\DZ{Need to discuss the remaining relevant static algorithms below.}
...
\PN{Adaptive Eviction Algorithms.}

An adaptive eviction algorithm involves 1) observing and characterizing 
workload access patterns and 2) adjusting its behavior accordingly~(by, \eg, automatically tuning internal parameters) to maximize performance. 
%
Existing adaptive algorithms fall into two categories: 1) size-adaptive ones 
and 2) policy-adaptive ones.
%

A size-adaptive algorithm partitions the cache into multiple parts and adapts by adjusting each part's size based on access patterns.
%
The representative examples are \arc~\cite{} and \car~\cite{}.
%
Both works follow the principle of dynamically balancing recency and frequency objects.
%
Specifically, \arc divides the cache into two LRU lists, L1 and L2.
%
L1 holds new coming objects for recency, while L2 keeps objects accessed more than once since admission for frequency.
%
They have two ghost caches to record the access history of evicted objects from L1 and L2, respectively.
%
If requested data is in the ghost cache of L1, it indicates that there is not enough space for recency data, and thus \arc increases the size of L1, vice versa.

% ARC divides the cache into two LRU lists, T1 and T2. T1 holds items accessed once while T2 keeps items accessed more than once since admission.


A policy-adaptive algorithm adapts by switching between multiple internal 
eviction algorithms according to their effectiveness under observed access 
patterns.
%
\cacheus~\cite{} and \lecar~\cite{} are the state of the art. 
%
When a new request arrives or hits in the cache, they update multiple metadata for each algorithm.
%
When the cache is full, they ask each algorithm to propose an eviction candidate.
%
If multiple algorithms propose different candidates, they adopt one of them based on a probability distribution and record the result in the ghost cache for future reference.
%
If requested data is in the ghost cache, it indicates that the corresponding algorithm makes a wrong eviction decision, and thus the algorithm's probability is decreased, vice versa.
%
