...
% This section provides background on classical access patterns~\cite{}~(\autoref{ss:access-pattern-based-analysis}) and on how prior work designs algorithms to cover them, both non-adaptively~\cite{} and adaptively~\cite{}~(\autoref{ss:non-adaptive-and-adaptive-algorithms}). However, the real-world access is much complex than ideal access patterns. We observe that most eviction algorithms operate in an unreliable metastable state, leading to a significant decrease in hit ratio for some benchmarks and configurations~(\autoref{ss:the-metastable-phenomenon}).


\subsection{Key Properties of Cache Eviction Algorithms}

\DZ{Add a paragraph here to state that software cache is widely used and then discusses the the following key properties.}
%
Software caches, e.g., Memcached~\cite{}, Redis~\cite{}, and the page cache~\cite{}, are widely adopted across various fields to improve service performance.
%
In the KV store and CDN servers, software caches in memory reduce data access latency and alleviate backend load.
%
In operating systems, the page cache stores frequent data in memory to speed up file perations.
%
To effectively enhance system performance, a cache eviction algorithm should possess the following key properties.

\PN{**Explain** and **motivate** each property below.}

\PN{High hit rate and Low latency.} 
%
A hit rate is the ratio of requests served from the cache to the total requests.
%
In the cache system, there is a fast cache layer and a slow backend layer. 
%
If a requested data is found in the cache, system responses quickly with low latency; otherwise, it has to fetch data from the slow backend, incurring high latency.
%
A good cache eviction algorithm should maximize the hit rate to minimize serving latency.

\PN{High throughput.}
%
A throughput is the number of requests processed per unit time~(QPS).
%
%A cache system often handles a large number of data requests per second.
%
A high throughput cache system can process many requests concurrently, ensuring that the cache does not become a bottleneck in the data access path.
%
Otherwise, server providers have to increase resources to handle the same workload.

\PN{Multicore Scalability.}
%
In an ideal scenario, throughput should scale linearly with the number of CPU cores.
%
Modern servers have multiple CPU cores, and for example, AMD EPYC 9965 processor has 192 cores~\cite{}.
%
With a multicore scalable cache eviction algorithm, the cache system can fully utilize the computing resources of the server to achieve high throughput.
%
However, updating metadata in a cache eviction algorithm often requires locks to ensure consistency, which limits multicore scalability.
%

\subsection{Static and Adaptive Cache Eviction Algorithms}

\DZ{Define these two types of algorithms.}

\PN{Static Eviction Algorithms.}
%
A static eviction algorithm has a fixed workflow whatever the access pattern is.
%
...
%
An adaptive eviction algorithm adjusts its workflow based on observed access patterns.
%
Typically, an adaptive algorithm has multiple states as static algorithms.
%
Cacheus~\cite{} and LeCaR~\cite{} invoke two basic static algorithms with different probabilities.
%
ARC~\cite{} and CAR~\cite{} separate data into different parts based on access patterns, and adjust the part sizes adaptively.
%
\sys changes pattern classification functions and prepares different workflows for different access patterns.
...
\PN{Cannot handle important access patterns.}
