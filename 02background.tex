...
To cover all the patterns, \cacheus have to switch its policy between SR-LRU and CR-LRU frequently.
%
However, SR-LRU evicts frequently accessed objects especially Churn, and CR-LRU prefers to evict data comes frist.
%
If none of sub-algorithm dominants, \cacheus suffers from confliction and degrades its performance.
%
\arc adjusts cache size for recency and frequency adaptively, but when the churn size is large, \arc cannot capture the frequently accessed objects effectively.
%
In this case, \sys captures them with two methods. 
%
First, \sys has a small cache to observe the access new objects, which avoids evicting them too early.
%
Second, \sys analysises the access patterns to find \cold \popular objects and holds \popular ones.

\PN{Why adaptiveness in \cacheus and \arc fails?}
%
\cacheus leverages two sub-algorithms to capture different access patterns and switches between them adaptively.
%
However, when multiple patterns coexist, the two sub-algorithms conflict with each other, degrading the performance.
%
...
%
It fails to identify complex patterns for \arc splitting objects into new objects and accessed more than once.
