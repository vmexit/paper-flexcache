...
\paragraph{Traces.} 
%
We used 11 open-source datasets with \XXX{wait for results} traces to analyze \sys. 
%
These traces are diverse, including key-value, object CDN, and block caches, spanning from 2007 to 2023.
%
\TODO{In total, these datasets contain ... requests, ... objects, ... TB of traffic for a total of ... TB of data.}
%
More details can be found in Table~\ref{tab:traces}.
%
In many large-scale distributed caching systems, traces in workloads are served by multiple servers for multi-tenant services.
%
Therefore, referring to previous work~\cite{yang2023fifo}, we split Tencent CBS~\cite{zhang2018tencent} and Alibaba~\cite{Alibaba} into per-tenant traces.
%

\paragraph{Metric.}
%
We evaluated different algorithms across various cache sizes for robustness and efficiency. 
%
Cache size ranges from 0.3\% to 20\% of the working set size (WSS), defined as the total number of objects in the trace. 
%
In some cases, the 0.3\% WSS is too small to be practical when the cache size is less than 1000 objects, so we ignore it.
%
%
...
%
With the help of slabs, we can ignore the impact of object size.
%

We evaluated 16 state-of-the-art algorithms and compare them with the dominant algorithm, which achieves the highest hit ratio for a specific trace with a particular cache size.
%
This can be used as a metric to demonstrate an algorithm's robustness.
%
Compared with the dominant algorithm, each policy has a relative hit ratio:$\frac{HR_{algo}}{HR_{dominant}}$ distribution for datasets in different cache sizes.
%
Additionally, we also calculate performance improvement relative to LRU:$\frac{HR_{algo} - HR_{lru}}{HR_{lru}}$. 
%
For datasets with more than 100 traces, we report both the distribution of relative hit rates and tail performance. 
%
For those with fewer than 100 traces, we only calculate the average performance.
%
To evaluate the efficiency of the cache utility, we also analyzed the byte miss ratio.

\paragraph{Simulator.} 
We implemented the \sys simulator in the libCacheSim~\cite{libcachesim} library.
%
For all state-of-the-art algorithms, we used the original implementations and parameters from their papers. 
%
We do not consider metadata size across algorithms, as \sys requires less metadata than production systems typically use.
%
...
\paragraph{Prototype.} 
%
Our prototype is implemented based on CacheLib~\cite{cachelib}, a production-level caching library developed by Meta, which employs slab memory management and has highly optimized the LRU-based replacement algorithm.
%
CacheLib uses extensive metaprogramming, many components are tightly coupled.
%
Therefore, we used linked lists to implement the filter, staging, and core queue, a hash table to implement the ghost queue, and a new atomic data structure sketch to implement the auxiliary store.
%
...
%
We have open-sourced our code with more details at the end of the paper.

\paragraph{Environment setup. }
%
The simulations used a server equipped with \XXX{the specifications of the kunpeng}.
%
The prototype experiments were conducted on a server with \XXX{the specifications of the avalon}. 
%
We pinned one thread to one core and used \texttt{numactl} to bind the experiments to a single NUMA node, ensuring consistent memory access latency. 
