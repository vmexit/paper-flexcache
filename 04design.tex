...
%
This section presents \sys's design goals~(\autoref{ss:design-goals}), an overview of \sys~(\autoref{ss:overview}), and concludes with a discussion~(\autoref{ss:discussion-and-limitations}).

\subsection{Design goals}
\label{ss:design-goals}

The design of \sys is guided by the following goals.

\PN{Cover most access patterns.}
...
\PN{Uses Queues to achieve multicore scalability.}
%
...
\squishlist
\item{{\ABB{F} filters out \textbf{infrequent-ephemeral} data quickly}}
\item{{\ABB{G} observes and suggests \textbf{frequent-ephemeral} data due to \ABB{F} space limitations for an epoch.}}
\item{{\ABB{K} records data at the end of an epoch to find \textbf{infrequent-persistent} data.}}
\item{{\ABB{M} holds most \textbf{frequent-persistent} data.}}
\item{{\ABB{S} checks data guessed by \sys and confirms pattern changes at the end of an epoch.}}
\squishend
