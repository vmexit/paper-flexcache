...
%
\sys integrates phase size with cache size and keeps data in the cache for at least one phase.
%
All the queues in \sys are FIFO-based which maintains the relative access order, so we only discuss the workflow between the queues for phase.
%
There are two phaseflows for \textit{frequent} and \textit{infrequent} data separately as in figure~\ref{fig:phaseflow}.
%
