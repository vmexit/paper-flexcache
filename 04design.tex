...
We implement a prototype of \sys in CacheLib~\cite{} for about 1,500 lines of code.
%
The storage logic leverages a hash table to index the position of data, which is efficiency and scalable to find keys.
%
The eviction algorithm logic for FIFO-based queues~(filer, core, staging) manages the order of data in the cache, which is linked-list implementation.
%
Linked-list pointers can integrate with storage logic with a hook portability.
%
\sys decouples insertion and eviction positions for scalability when the queue is about to full.
%
The eviction algorithm logic evicts data from the tail of the queue, and inserts new data to the head of the queue with atomic compare and exchange operation.
%
...
\ABB{G} can be further simplified for efficiency and scalability without linked-list pointers.
%
\sys compresses the metadata in \ABB{G} to only a fingerprint, a timestamp, and a \hotness field in 8-bytes.
%
\sys stores metadata in a closed hash table with multiple slots.
%
If the fingerprint matches, \sys updates the \hotness field for hotness awareness as a hit in \ABB{G}.
%
Otherwise, \sys increases the global timestamp for epoch awareness.
%
\sys reserves slots whose timestamp ranges from $timestamp-\#epoch\_size$ to $timestamp$.
%
If all slots are occupied, \sys evicts a random slot for collision resolution.
