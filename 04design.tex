...
\begin{figure}[t]
    ...
    \caption{Overview for \sys. Cache space consists of five parts: filter cache, main cache, suspected cache, ghost cache, and CBF. The workflow has three major parts, (1)cache promotion and eviction in four sub-cache. (2)IA-MP objects in the filter cache duels with the tail object in the suspected cache with CBF. (3)Leverage the metadata in the ghost cache to find FA-FP objects and prefetch sequence accessed objects.}
    \label{fig:overview}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{\sys consists of two phaseflows.(1)Frequently accessed objects' movement from the main cache to the suspected cache and reinsertion to the main cache. (2)Infrequently accessed objects' movement from the filter cache to the ghost cache. Objects' type changes with suspection in the filter and ghost cache.}
    \label{fig:phaseflow}
\end{figure}
...
%
As shown in figure~\ref{fig:phaseflow}, phase changing happens in the suspected cache with the aging function.
%
...
%
FA-MP objects contributes the most to the hit ratio~(\autoref{ss:dynamic-adjust}), and suspected FA-FP and IA-MP objects are small part of the suspected cache.
%
To make efficient use of space, \sys treats the main cache and the suspected cache together as the phase duration for frequently accessed data. 
%
...
%
When sequenced object accesses occur, \sys prefetch the next object from the ghost cache to the filter cache, which separates suspected objects with hits from the suspected objects without access.
%
...
%
When an object is about to leave the filter cache, \sys compares its access record in the CBF with the access record of the tail object in the suspected cache.
...
%
To make it timely, \sys records the objects when they leave the filter cache or suspected cache.
%
...
%
As mentioned in figure~\ref{fig:phaseflow}, \sys involves two types of phase-aware objects handling: (1)frequently accessed objects undergo phase aging in the main cache and suspected cache~(\autoref{ss:filter-hold}). (2)the ghost cache handles infrequently accessed objects with phase awareness~(\autoref{ss:ghost}).
%
...
%
Third, the CBF records the phase information, its dueling between the filter and suspected cache will disturb data calcification.

Suspected objects may disrupt the relative order, so \sys duplicates the metadata as a placeholder in figure~\ref{fig:phaseflow}.
%
...
%
If we wrongly promote the suspected objects, and then evict them back to the ghost cache, they persists for a longger phase.
%
Therefore, \sys duplicates the metadata for suspected objects to avoid disrupting the relative order of phases.
%
If they are truely FA-MP objects, \sys discards the metadata in the ghost cache and promotes them to the main cache.

\sys decouples the suspected objects \textbf{with hits} from the suspected objects \textbf{without access}.
%
The suspected objects with hits are promoted to the suspected cache, and it will become a FA-MP object if it is accessed again.
%
While \sys prefetches the suspected objects if prior requests access in sequence.
% 
