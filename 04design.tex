...
%
Finally, \ABB{K} conservatively proposes \textit{infrequent-persistent} data and leverages a forget function to forget old records beyond many phases.
...
Almost all type transitions happen in \ABB{S}, while \ABB{M} only stores data for phase analysis~(\BC{7}).
%
...
%
If \textit{frequent-ephemeral} data from \ABB{M} is alive in the new phase, \sys transits it to \textit{frequent-persistent} type~(\BC{8}).
%
...
%
\sys evicts data without access for many phases based on hotness counter and does not change their types.
...
%
\sys stores a scope of timestamps based on the phase size and evicts data beyond the scope.
...
%
A sliding window sketch~\cite{} provides various forget functions, which separates \ABB{K} into many sub-sketches for different phases.
%
At the beginning of a new phase, it inserts records into a new sub-sketch and forgets the oldest sub-sketch beyond many phases.
%
...
\textbf{Online Adaption.} \sys integrates online phase and hotness analysis with cache size to adapt to various access patterns.
%
With different cache sizes, data experiences different phase sizes in \ABB{M} and \ABB{G}, and hotness threshold changes with phase.
%
Therefore, to adjust the phase size and hotness threshold, it is convenient to change the size of \ABB{M} and \ABB{G} and the target portion.
%
...
%
In Count-Min Sketch~\cite{}, with a false positive error rate of 1\% and error range of 1, a 8-phase \textit{infrequent-persistent} data requires about 3 bits for record.
%
...
%
As discussed in \autoref{ss:overview}, \ABB{G} also keeps the relative access order for phase awareness, which provides an opportunity for prefetching.
%
...
%
\sys highly relies on phase and hotness analysis to identify access patterns.
%
Workloads with large reuse which apporaches the phase size are challenge for phase analysis.
%
The large reuse would have many hits in \ABB{G} rather than \ABB{M} for the first phase in \sys, and it will be evicted by suspected data in \ABB{S} at the end of the phase, leading to many misses.
%
Workloads with many equal hotness counters are challenge for hotness analysis.
%
In this case, the hotness threshold would cut through many data, and \sys have to discard or pick up some data.
%
For example, many data accessed only once or twice in a phase, and \ABB{K} suspects a part of them as useful data.
%
