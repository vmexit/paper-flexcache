...
%
To receive a timely and accurate access pattern, \sys works in an online optimization with FIFO queues and a sketch for phase classification~(\autoref{ss:adaptive-phases}) and hotness distribution for hotness classification~(\autoref{ss:hotness-during-phases}).
...
%
\BC{1} For phase analysis, \ABB{K} records all the data movement out of \ABB{F} and checked data by \ABB{S} with a forget function, and we will discuss it later.
%
\BC{2} For hotness analysis, \sys updates the hotness distribution and adjust the hotness threshold field whenever a cache hit or eviction occurs, and we will discuss it later.
%
\BC{3} All new data not in the cache are inserted into \ABB{F}.
%
\BC{4} Eviction in \ABB{F}
%
\BC{5} If hit occurs in \ABB{G}, \sys promotes \textit{frequent-ephemeral} data to \ABB{M} and susppects some \textit{frequent-ephemeral} data.
%
\BC{6} \TODO{suspect in \sys}
\sys moves the suspected data to \ABB{S} for further examination and keeps the duplicated metadata in \ABB{G} for phase analysis.
%
\BC{7} \TODO{hit and eviction in M}
%
\BC{8} \TODO{check in S}
\BC{9} \TODO{suspected in S}
...
\textbf{Hotness in \sys.}
\sys increases the hotness counter by one when a hit occurs, and only decrease the hotness counter when the object is evicted from \ABB{G} or ages in \ABB{S}.

\textbf{Type changes in \sys.}
...
%
%
\BC{1} All new objects objects are inserted into the filter cache.
%
